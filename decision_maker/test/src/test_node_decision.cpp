...
TEST_F(TestSuite, isArrivedGoal)
{
  ...
  ASSERT_TRUE(test_obj_.isArrivedGoal())
    << "Current pose is outside the target range."
    << "It should be true";

  test_obj_.setCurrentPose(100, 0, 0);
  test_obj_.setCurrentVelocity(3.0);
  ASSERT_FALSE(test_obj_.isArrivedGoal())
    << "Current pose is inside the target range."
    << "It should be false";
